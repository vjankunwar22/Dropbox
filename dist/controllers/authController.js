...
exports.register = (0, helpers_1.tryCatchHandler)(async (req, res) => {
    ...
    if (!email || !password) {
        throw new error_1.HttpError("BAD_REQUEST", "Email and password is required");
    }
    const existingUser = await db_config_1.default.user.findUnique({ where: { email } });
    if (existingUser) {
        throw new error_1.HttpError("BAD_REQUEST", "User Already Exist");
    }
...
exports.login = (0, helpers_1.tryCatchHandler)(async (req, res) => {
    ...
    if (!user || !user.password) {
        throw new error_1.HttpError("UNAUTHORIZED", "User not found .");
    }
    const isMatch = await bcrypt_1.default.compare(password, user.password);
    if (!isMatch) {
        throw new error_1.HttpError("UNAUTHORIZED", "Invalid Credentials");
    }
